...
def test_get_log_index(client):
    """Expects to get and empty index."""
    create_user(client, 'userA', 'passA')
    client_id = create_client(client)
    access_token = create_token(client, 'userA', 'passA', client_id)

    rv = client.get('/v1/log', follow_redirects=True, headers={
        'Authorization': 'Bearer ' + access_token
    })
...
def test_get_nonexistent_log(client):
    ...
    rv = client.get('/v1/log/1', follow_redirects=True, headers={
        'Authorization': 'Bearer ' + access_token
    })
...
def test_create_log(client):
    ...
    # Create the item.
    rv = client.post('/v1/log', follow_redirects=True, headers={
        'Authorization': 'Bearer ' + access_token
    }, data={
        'public': 0,
        'message': 'Hello world!',
...
def test_update_log(client):
    ...
    rv = client.put('/v1/log/1', follow_redirects=True, headers={
        'Authorization': 'Bearer ' + access_token
    }, data={
        'etag': etag + 'a',
        'public': 1,
...
def test_delete_log(client):
    ...
    # Try a wrong etag and expect an error.
    rv = client.delete('/v1/log/1', follow_redirects=True, headers={
        'Authorization': 'Bearer ' + access_token
    }, data={
        'etag': etag + 'a',
    })
...
def test_pagination(client):
    ...
    rv = client.get(
        '/v1/log?page=2&max_results=5&sort=id-desc', follow_redirects=True,
...
def test_search(client):
    ...
    rv = client.get(
        '/v1/log?search=' + search, follow_redirects=True,
        headers={'Authorization': 'Bearer ' + access_token})
    ...
    rv = client.get(
        '/v1/log?search=' + wrong_column_operator, follow_redirects=True,
        headers={'Authorization': 'Bearer ' + access_token})
    ...
    rv = client.get(
        '/v1/log?search=' + search_text, follow_redirects=True,
        headers={'Authorization': 'Bearer ' + access_token})
...
def test_complex_search(client):
    ...
    rv = client.get(
        '/v1/log?search=' + wrong_condition_operator, follow_redirects=True,
        headers={'Authorization': 'Bearer ' + access_token})
